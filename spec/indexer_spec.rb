...
RSpec.describe Jsonar::Indexer do
  describe 'build_index' do
    it 'works for an array of objects with string values' do
      @json = '[{"foo": "bar"}]'
      expect(Jsonar::Indexer.build_index(@json)).to eq('bar' => { 'foo' => 'bar' })
    end

    it 'works for an array of objects with integer values' do
      @json = '[{"foo": 3}]'
      expect(Jsonar::Indexer.build_index(@json)).to eq(3 => { 'foo' => 3 })
    end

    it 'works for an array of objects with boolean values' do
      @json = '[{"foo": true}]'
      expect(Jsonar::Indexer.build_index(@json)).to eq(true => { 'foo' => true })
    end

    it 'works for an array of objects with null values' do
      @json = '[{"foo": null}]'
      expect(Jsonar::Indexer.build_index(@json)).to eq(nil => { 'foo' => nil })
    end

    it 'works for an array of objects with array fields' do
      @json = '[{"foo": ["bar", "baz"]}]'
      expect(Jsonar::Indexer.build_index(@json)).to eq('bar' => { 'foo' => %w[bar baz] },
                                                       'baz' => { 'foo' => %w[bar baz] })
    end
